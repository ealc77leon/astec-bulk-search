...
async function loadCotDB(){
  ...
  try{
    ...
    // Columnas Python (astec_pdf_to_sheets.py):
    // Quote#(0) Fecha(1) Quote Date(2) Expires(3) Line(4) Part(5)
    // Description(6) Expected Qty(7) Unit Price(8) Ext. Price(9) Lead Time(10) Notes(11)
    //
    // — detección automática por header —
    var hdr=rows[0].map(function(c){return String(c).trim().toLowerCase();});
    var COL={
      quote:    hdr.indexOf('quote#'),
      fecha:    hdr.indexOf('fecha'),
      line:     hdr.indexOf('line'),
      part:     hdr.indexOf('part'),
      desc:     hdr.indexOf('description'),
      qty:      hdr.indexOf('expected qty'),
      price:    hdr.indexOf('unit price'),
      extPrice: hdr.indexOf('ext. price'),
      leadTime: hdr.indexOf('lead time'),
      notes:    hdr.indexOf('notes')
    };
    // fallback a índices fijos si el header no coincide
    if(COL.part<0) COL.part=5;
    if(COL.quote<0) COL.quote=0;
    if(COL.fecha<0) COL.fecha=1;
    if(COL.line<0) COL.line=4;
    if(COL.desc<0) COL.desc=6;
    if(COL.qty<0) COL.qty=7;
    if(COL.price<0) COL.price=8;
    if(COL.extPrice<0) COL.extPrice=9;
    if(COL.leadTime<0) COL.leadTime=10;
    if(COL.notes<0) COL.notes=11;

    for(var i=1;i<rows.length;i++){
      var r=rows[i];
      if(r.length<6)continue;
      var part=String(r[COL.part]||'').trim().toUpperCase();
      if(!part)continue;
      if(!cotDB[part])cotDB[part]=[];
      cotDB[part].push({
        quote:    String(r[COL.quote]   ||'').trim(),
        fecha:    String(r[COL.fecha]   ||'').trim(),
        line:     String(r[COL.line]    ||'').trim(),
        desc:     String(r[COL.desc]    ||'').trim(),
        qty:      String(r[COL.qty]     ||'').trim(),
        price:    String(r[COL.price]   ||'').trim(),
        extPrice: String(r[COL.extPrice]||'').trim(),
        leadTime: String(r[COL.leadTime]||'').trim(),
